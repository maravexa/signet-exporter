...
func TestScheduler_ImmediateFirstScan(t *testing.T) {
	...
	sched := NewScheduler([]Scanner{mock}, store, []SubnetConfig{subnet}, 2, nil)
...
func TestScheduler_ConcurrencyLimit(t *testing.T) {
	...
	sched := NewScheduler([]Scanner{tracker}, store, subnets, maxParallel, nil)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestScheduler_ReadyAfterAllSubnets(t *testing.T) {
	...
	sched := NewScheduler([]Scanner{mock}, store, subnets, 3, nil)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestScheduler_MultipleScannersPerSubnet(t *testing.T) {
	...
	sched := NewScheduler([]Scanner{arp, icmp}, store, []SubnetConfig{subnet}, 2, nil)
